...
public abstract class RecursiveRendererBase<TOutput> : IRenderer<TOutput> {
	...
	protected RecursiveRendererBase(RenderMode renderMode, ILocalNotionRepository repository, IThemeManager themeManager, ILinkGenerator resolver, IBreadCrumbGenerator breadCrumbGenerator, ILogger logger) {
	...
	protected ILocalNotionRepository Repository { get; set; }

	protected IThemeManager ThemeManager { get; set; }
	...
	public virtual TOutput Render(LocalNotionEditableResource page) {
		var themes = Repository.CalculateApplicableThemes(page);
		...
		RenderingContext = new PageRenderingContext {
			Resource = page,
			PageGraph = visualGraph,
			PageObjects = visualObjects,
			Themes = htmlThemes
		};
		OnRenderingContextCreated(RenderingContext);
		return Render(RenderingContext.PageGraph);
	}

	protected virtual void OnRenderingContextCreated(PageRenderingContext context) {
	}
	...
	protected virtual TOutput Render(NotionObjectGraph objectGraph, int? index = null) {
		...
		try {
			return RenderingContext.CurrentRenderingObject switch {
				...
				Page x => RenderingContext.RenderingStack.Count == 1 ? Render(x) : Render(x.AsChildPageBlock()),   // Nested pages are stored as "Page" objects in LocalNotion
				User x => Render(x),
	...
	protected virtual TOutput Render(IPageIcon pageIcon)
		=> pageIcon switch {
			...
			ExternalFile externalFile => (TOutput)(object)GetFileUrl(externalFile, out _),   // WARNING: ugly cast hack here
			UploadedFile uploadedFile => (TOutput)(object)GetFileUrl(uploadedFile, out _),   // WARNING: ugly cast hack here
