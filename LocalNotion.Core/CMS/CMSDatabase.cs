...
public class CMSDatabase : ICMSDatabase {
	...
	public bool TryGetComponentPage(string slug, CMSPageType pageType, out LocalNotionPage page) {
		var contentNode = _contentHierarchy[slug];
		page = contentNode?.Content.FirstOrDefault(x => x.CMSProperties.PageType == pageType);
		return page != default;
	}

	public bool FindComponentPage(string slug, CMSPageType pageType, out LocalNotionPage page) {
		// slug specific footer
		if (TryGetComponentPage(slug, pageType, out page))
			return true;

		// default footer
		if (TryGetComponentPage(string.Empty, pageType, out page))
			return true;
	...
	protected virtual CMSContentType DetermineContentType(CMSContentNode node) {
		...
		var consumableNodeContent = node.Content.Where(CMSHelper.IsContent).ToArray(); // filter out menus/footers
		...
		if (node.Visit(n => n.Children).SelectMany(n => n.Content).Where(CMSHelper.IsContent).All(x => x.CMSProperties.PageType == CMSPageType.Gallery))
	...
	private Dictionary<string, CMSContentNode> FetchContentHierarchy(string cmsDatabaseID, bool publishedOnly) {
		...
		CMSContentNode GetOrCreateNode( IEnumerable<(string slugPart, string partTitle)> slugParts, string slugOverride = null) {
			...
			var slug = Tools.Url.StripAnchorTag( slugOverride ?? CMSHelper.CalculateSlug(slugPartsArr.Select(x => x.slugPart)) );
	...
	private IDictionary<string, string[]> FetchCategoriesByCategorySlug() {
		...
		foreach (var page in Repository.Resources.Where(r => r.Type == LocalNotionResourceType.Page).Cast<LocalNotionPage>()) {
			...
			if (string.IsNullOrWhiteSpace(page.CMSProperties.Category1))
				continue;
			result.Add(CMSHelper.CalculateSlug(new[] { page.CMSProperties.Root, null, null, null, null, null }), page.CMSProperties.Category1);

			if (string.IsNullOrWhiteSpace(page.CMSProperties.Category2))
				continue;
			result.Add(CMSHelper.CalculateSlug(new[] { page.CMSProperties.Root, page.CMSProperties.Category1, null, null, null, null }), page.CMSProperties.Category2);

			if (string.IsNullOrWhiteSpace(page.CMSProperties.Category3))
				continue;
			result.Add(CMSHelper.CalculateSlug(new[] { page.CMSProperties.Root, page.CMSProperties.Category1, page.CMSProperties.Category2, null, null, null }), page.CMSProperties.Category3);

			if (string.IsNullOrWhiteSpace(page.CMSProperties.Category4))
				continue;
			result.Add(CMSHelper.CalculateSlug(new[] { page.CMSProperties.Root, page.CMSProperties.Category1, page.CMSProperties.Category2, page.CMSProperties.Category3, null, null }), page.CMSProperties.Category4);

			if (string.IsNullOrWhiteSpace(page.CMSProperties.Category5))
				continue;
			result.Add(CMSHelper.CalculateSlug(new[] { page.CMSProperties.Root, page.CMSProperties.Category1, page.CMSProperties.Category2, page.CMSProperties.Category3, page.CMSProperties.Category4, null }), page.CMSProperties.Category5);
	...
	private IDictionary<string, string[]> FetchPagesByCategorySlug() {
		...
		foreach (var page in Repository.Resources.Where(r => r is LocalNotionPage { CMSProperties: not null }).Cast<LocalNotionPage>()) {
			...
			var categoryKey = CMSHelper.CalculateSlug(
				new[] {
			...
			if (!string.IsNullOrWhiteSpace(page.CMSProperties.Category5)) {
				categoryKey = CMSHelper.CalculateSlug(
			new[] {
					page.CMSProperties.Root,
					page.CMSProperties.Category1,
					page.CMSProperties.Category2,
